...
try {
	...
	if (file_exists(realpath(__DIR__) . '/cache/config.php')) {
		require_once(realpath(__DIR__) . '/cache/config.php');
	} else {
		if (!file_exists(realpath(__DIR__) . '/config.ini')) {
			throw new \Exception("Please configure your lib/config.ini file before installing the application");
		} else {
			$ini = parse_ini_file(realpath(__DIR__) . '/config.ini');
			$buffer = array();
			$buffer[] = '<?php';
			...
			$tmp    = "define('%s', '%s');";
			$buffer[] = sprintf($tmp, 'LIB_DIR', realpath(__DIR__));
			foreach ($ini as $key => $value) {
				$buffer[] = sprintf($tmp, strtoupper($key), $value);
			}
			if (!isset($ini['COOKIE_NAME'])) {
				$buffer[] = sprintf($tmp, 'COOKIE_NAME', 'gunLocal');
			}
			if (!isset($ini['OFFER_KEY'])) {
				...
				if (strpos($_SERVER['DOCUMENT_ROOT'], '/docroot') !== false) {
					...
					$buffer[] = sprintf($tmp, 'OFFER_KEY', $offer_key);
				} else {
					throw new \Exception('You must have your path within a docroot/ folder');
				}
			}
			$buffer[] = '?>';
			file_put_contents(realpath(__DIR__) . '/cache/config.php', implode("\n", $buffer));
			require_once(realpath(__DIR__) . '/cache/config.php');
		}
	}
