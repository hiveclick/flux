...
// +----------------------------------------------------------------------------+
class ManualFulfillRosieHipAction extends BasicRestAction
{
    ...
    function executePost($input_form) {
        ...
        if (!is_null($lead->getId())) {
            ...
            $buffer[] = 'Please review Hip lead information below:';
            $buffer[] = '';
            ...
            try {
                ...
                $lead->addNote('Lead successfully sent to Rosie as Hip');
                $lead->setValue('fulfilled', 1);
                $lead->update();
            } catch (\Exception $e) {
                $this->getErrors()->addError('error', $e->getMessage());
                $lead->addNote('Lead unsuccessfully sent to Rosie as Hip: ' . $e->getMessage());
                $lead->update();
            }
        }
        return $ajax_form;
