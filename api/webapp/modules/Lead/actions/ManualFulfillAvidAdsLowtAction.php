...
// +----------------------------------------------------------------------------+
class ManualFulfillAvidAdsLowtAction extends BasicRestAction
{
    ...
    function executePost($input_form) {
        ...
        if (!is_null($lead->getId())) {
            ...
            $params['campaign_id'] = 482;
            $params['posting_token'] = '4c33d2d03ad9847cb696596ca5fce398';
            ...
            $params['drug_used'] = implode(",", $lead->getValue('drug_brand'));
            $side_effect = array_shift($lead->getValue('effect_type'));
            $params['side_effect'] = $side_effect;
            $params['year_of_use'] = $lead->getValue('year_of_use');
            $params['age'] = implode(",", $lead->getValue('age'));
            ...
            if (strpos($response->getBody(), 'Good') === false) {
    			...
    			$lead->addNote('Lead unsuccessfully sent to Avid Ads as Low-T: ' . $response->getBody() . '<br /><small>' . $url . '?' . http_build_query($params, null, '&') . '</small>');
    			$lead->update();
    		} else {
    		    $lead->addNote('Lead successfully sent to Avid Ads as Low-T');
