<?php
    /* @var $client \Gun\Client */
    $client = $this->getContext()->getRequest()->getAttribute("client", array());
?>
<div class="help-block">Add a new user to the system that can login and manage offers on this client</div>
<br/>
<table id="client_user_table" class="table table-hover table-bordered table-striped table-responsive">
    <thead>
        <tr>
            <th>Email</th>
            <th>Name</th>
            <th>Type</th>
            <th>Status</th>
        </tr>
    </thead>
    <tbody>
    </tbody>
</table>
<form id="client_user_search_form" method="GET" action="/api">
    <input type="hidden" name="func" value="/admin/user">
    <input type="hidden" name="client_id" value="<?php echo $client->getId() ?>">
</form>
<script>
//<!--
$(document).ready(function() {
    $('#client_user_table').DataTable({
    	autoWidth: false,
    	serverSide: true,
		pageLength: 15,
		ajax: $.fn.dataTable.pageCache({
            url: '/api',
            data: $('#client_user_search_form').serializeObject()
        }),
    	searching: false,
    	paging: true,
    	dom: 'Rfrtpi',
    	columns: [
            { name: "_email", data: "email", createdCell: function (td, cellData, rowData, row, col) {
                $(td).html('<a href="/admin/user?_id=' + rowData._id + '">' + rowData.email + '</a>');
            }},
            { name: "client_id", data: "_client_name", createdCell: function (td, cellData, rowData, row, col) {
                $(td).html('<a href="/client/client?_id=' + rowData.client_id + '">' + cellData + '</a>');
            }},
            { name: "user_type", data: "_user_type_name" },
            { name: "status", data: "_status_name" }
      	]
    });
});
//-->
</script>