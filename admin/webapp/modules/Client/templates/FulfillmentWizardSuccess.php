...
<div class="modal-header">
	...
	<h4 class="modal-title"><?php echo ($client_export->getId() > 0) ? 'Edit' : 'Add' ?> Fulfillment Handler</h4>
</div>
<form class="" id="client_export_form_<?php echo $client_export->getId() ?>" method="<?php echo ($client_export->getId() > 0) ? 'PUT' : 'POST' ?>" action="/api" autocomplete="off" role="form">
	<input type="hidden" name="func" value="/client/client-export" />
	<input type="hidden" name="status" value="<?php echo \Flux\ClientExport::CLIENT_EXPORT_STATUS_ACTIVE ?>" />
	<?php if ($client_export->getId() > 0) { ?>
		...
		<!-- Tab panes -->
  		<div class="tab-content">
  			<div role="tabpanel" class="tab-pane fade in active" id="basic">
				...
				<div id="export_settings" class="<?php echo $client_export->getExportClass()->getClientExportType() == 0 ? 'hidden' : ''; ?>">
					...
					<div class="form-group">
						<select class="form-control" name="client_id" id="client_id" required="required" placeholder="Assign an owner to this handler...">
				...
				<div id="ftp_settings" class="<?php echo $client_export->getExportClass()->getClientExportType() != \Flux\Export\ExportAbstract::CLIENT_EXPORT_TYPE_FTP ? 'hidden' : ''; ?>">
					...
					<div class="form-group">
						<label class="control-label" for="ftp_hostname">FTP Host/Port</label>
				...
				<div id="post_settings" class="<?php echo $client_export->getExportClass()->getClientExportType() != \Flux\Export\ExportAbstract::CLIENT_EXPORT_TYPE_POST ? 'hidden' : ''; ?>">
					<div class="help-block">Enter the POST url and click parse url to check it</div>
					<div class="form-group">
					...
					?>
						<div class="form-group map-group-item">
							...
							<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
								<input type="text" id="mapping_<?php echo $counter ?>" name="mapping[<?php echo $counter;?>][field_name]" class="form-control" value="<?php echo $client_export_map->getFieldName() ?>" placeholder="POST field name (required)" />
							...
							<div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 text-right">
								<a class="btn btn-info map_options-<?php echo $counter ?>" type="button" data-toggle="modal" data-target="#map_options_modal" href="/client/fulfillment-pane-map-options-modal?column_id=<?php echo $counter ?>">Options</a>
...
$(document).ready(function() {
	$('#client_export_form_<?php echo $client_export->getId() ?>').form(function(data) {
	...
	$('#status,#export_type,#client_id').selectize();
	...
	$('#export_class_name').change(function() {
		...
		<?php foreach($export_handlers AS $export_class_name => $export_class_instance) { ?>
			if ($('#export_class_name').val() == '<?php echo $export_class_name ?>') {
				...
				<?php } else if ($export_class_instance->getClientExportType() == \Flux\Export\ExportAbstract::CLIENT_EXPORT_TYPE_EMAIL) { ?>
		 			$('#email_settings').removeClass('hidden');
				<?php } else if ($export_class_instance->getClientExportType() == \Flux\Export\ExportAbstract::CLIENT_EXPORT_TYPE_POST) { ?>
...
function confirmDelete() {
	if (confirm('Are you sure you want to delete this user from the system?')) {
		$.rad.del({ func: '/client/client-export/<?php echo $client_export->getId() ?>' }, function(data) {
