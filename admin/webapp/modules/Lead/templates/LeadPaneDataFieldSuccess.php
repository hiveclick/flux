	...
	<div class="col-sm-6">
		<select name="posting_url_dataField_name[]" class="form-control selectize">
			...
			<?php foreach(\Flux\DataField::retrieveActiveDataFields() AS $dataFieldId => $dataField) { 
					...
					array_walk($data_field_set, function(&$value) { if ($value instanceof \Flux\DataFieldSet) { $value = $value->toArray(); }});
			?>
				<?php if ($dataField->getStorageType() == \Flux\DataField::DATA_FIELD_STORAGE_TYPE_DERIVED) { ?>
					<option value="<?php echo $dataField->getKeyName() ?>" data-data="<?php echo htmlentities(json_encode(array('name' => $dataField->getName(), 'key_name' => $dataField->getKeyName(), 'description' => $dataField->getDescription(), 'data_field_set' => $data_field_set, 'request_names' => array_merge(array($dataField->getKeyName(), $dataField->getRequestName()))))) ?>"><?php echo $dataField->getName() ?> (<?php echo $dataField->getKeyName() ?>, <?php echo implode(", ", $dataField->getRequestName()) ?>)</option>
				<?php  } ?>
			<?php } ?>
			</optgroup>
		...
		<div id="dataField_posting_url_container">
			...
			?>
				<div class="form-group">
					<div class="col-sm-6">
						<select name="posting_url_dataField_name[]" class="form-control selectize">
							<optgroup label="Data Fields">
								...
								?>
									<?php if ($dataField->getStorageType() == \Flux\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT) { ?>
										<option value="<?php echo $dataField->getKeyName() ?>" <?php echo $data_field['posting_url_dataField_id'] == $dataField->getId() ? 'selected="selected"' : '' ?> data-data="<?php echo htmlentities(json_encode(array('name' => $dataField->getName(), 'key_name' => $dataField->getKeyName(), 'description' => $dataField->getDescription(), 'data_field_set' => $data_field_set, 'request_names' => array_merge(array($dataField->getKeyName(), $dataField->getRequestName()))))) ?>"><?php echo $dataField->getName() ?> (<?php echo $dataField->getKeyName() ?>, <?php echo implode(", ", $dataField->getRequestName()) ?>)</option>
									<?php } ?>
								<?php } ?>
							</optgroup>
							<optgroup label="Derived">
					...
					<div class="col-sm-4">
						<textarea name="posting_url_dataField_value[]" class="form-control selectize-text" placeholder="Enter New Value" rows="3"><?php echo $data_field['posting_url_dataField_value'] ?></textarea>
					</div>
					<div class="col-sm-2">
						<button type="button" class="btn btn-sm btn-danger btn-remove-dataField">
							<span class="glyphicon glyphicon-minus"></span>
...
$(document).ready(function() {
	...
	$('#dataField_posting_url_container .selectize-text').selectize({
		valueField: 'value',
		labelField: 'name',
		searchField: ['name'],
		sortField: 'name',
	...
	$('#dataField_posting_url_container .selectize').selectize($selectize_options);
	$('#dataField_posting_url_container .selectize').each(function(i, item) {
		if ($(this)[0].selectize == undefined) return;
		var select_text_obj = $(this).closest('.form-group').find('.selectize-text');
		var select_text = $(this).closest('.form-group').find('.selectize-text')[0].selectize;
