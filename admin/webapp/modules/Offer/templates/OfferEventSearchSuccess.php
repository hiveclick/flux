...
<div class="page-header">
	<h1>Offer Events <small>Manage events for <?php echo $offer->getName() ?></small></h1>
</div>
<ol class="breadcrumb">
		...
		<div class="form-group event-group-item">
			<div class="col-sm-4">
				<select name="events[<?php echo $counter;?>][event_id]" class="form-control selectize">
					<?php
						/* @var $event \Flux\DataField */ 
						foreach($data_fields AS $event) { 
					?>
						<?php if ($event->getStorageType() == \Flux\DataField::DATA_FIELD_STORAGE_TYPE_EVENT) { ?>
