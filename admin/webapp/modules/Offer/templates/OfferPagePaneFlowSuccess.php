...
<div class="help-block">Use this page to define where a user will be taken when they leave this page</div>
<br/>
...
<div class="form-group flow-group-item" style="display:none;" id="dummy_pageflow_div">
    ...
    <div class="col-sm-4">
        <div class="panel panel-default">
            <div class="panel-heading">
                <div class="pull-right">
                    <a href="#" class="btn-sm btn-danger btn_remove_rule">remove rule</a>
                </div>
                <h3 class="panel-title">Navigation</h3>
            </div>
            <div class="panel-body">
                <select class="offer_page_flow_navigation" name="offer_page_flows[dummy_pageflow_id][destination_offer_page_id]" class="selectize">
                    <?php 
                        /* @var $page \Gun\OfferPage */
                        foreach ($offer_pages as $page) {
                    ?>
                        <option value="<?php echo $page->getId() ?>" data-data="<?php echo htmlentities(json_encode(array('_id' => $page->getId(), 'name' => $page->getName(), 'description' => $page->getDescription(), 'page_name' => $page->getPageName(), 'preview_url' => $page->getPreviewUrl()))) ?>"><?php echo $page->getName() ?></option>
                    <?php } ?>
                </select>
                <p />
                <a class="btn-sm btn-info text-center manage_destination_page" target="_blank" href="#">manage destination page &gt;&gt;</a>
            </div>
        </div>
    </div>
...
<form class="form-horizontal" id="offer_page_flow_form" name="offer_page_flow_form" method="PUT" action="/api" autocomplete="off" role="form">
            ...
            <div class="form-group flow-group-item">
                        ...
                        <div class="panel-body">
                            <div class="offer_page_flow_filter_div-<?php echo ($key + 1) ?>">
                                <?php if (count($offer_page_flow->getFilterConditions()) > 0) { ?>
                                    <?php if ($offer_page_flow->getFilterType() == \Gun\OfferPageFlow::FILTER_TYPE_ALL) { ?>
                                    ...
                                    <?php foreach ($offer_page_flow->getFilterConditions() as $filter_key => $filter_condition) { ?>
                                        <li>
                                            ...
                                            <em>
                                                <?php if ($filter_condition->getFilterOp() == \Gun\OfferPageFlowFilter::FILTER_OP_CONTAINS) { ?>
                                                    contains
                                                <?php } else if ($filter_condition->getFilterOp() == \Gun\OfferPageFlowFilter::FILTER_OP_BEGINS_WITH) { ?>
                                                    begins with
                                                <?php } else if ($filter_condition->getFilterOp() == \Gun\OfferPageFlowFilter::FILTER_OP_ENDS_WITH) { ?>
                                                    ends with
                                                <?php } else if ($filter_condition->getFilterOp() == \Gun\OfferPageFlowFilter::FILTER_OP_IS) { ?>
                                                    is
                                                <?php } ?>
                            ...
                            <div class="offer_page_flow_setter_div-<?php echo ($key + 1) ?>">
                                <?php if (count($offer_page_flow->getSetters()) > 0) { ?>
                                    ...
                                    <ul>
                                        ...
                                        <li>
                                            <em>
                                                <?php if ($setter->getSetterOp() == \Gun\OfferPageFlowSetter::SETTER_SET) { ?>
                                                    Set
                                                <?php } else if ($setter->getSetterOp() == \Gun\OfferPageFlowSetter::SETTER_SET_IF_EMPTY) { ?>
                                                    Set (if blank)
                                                <?php } else if ($setter->getSetterOp() == \Gun\OfferPageFlowSetter::SETTER_INCREMENT) { ?>
                                                    Increment
                                                <?php } else if ($setter->getSetterOp() == \Gun\OfferPageFlowSetter::SETTER_DECREMENT) { ?>
                                                    Decrement
                                                <?php } ?>
                                            </em>
                                            ...
                                            <input type="hidden" name="offer_page_flows[<?php echo ($key + 1) ?>][setters][<?php echo ($filter_key + 1) ?>][data_field_id]" value="<?php echo $setter->getDataFieldId() ?>" />
                                            <input type="hidden" name="offer_page_flows[<?php echo ($key + 1) ?>][setters][<?php echo ($filter_key + 1) ?>][setter_op]" value="<?php echo $setter->getSetterOp() ?>" />
                                            <input type="hidden" name="offer_page_flows[<?php echo ($key + 1) ?>][setters][<?php echo ($filter_key + 1) ?>][setter_value]" value="<?php echo $setter->getSetterValue() ?>" />
                                        </li>
                                    <?php } ?>
                                    </ul>
                                <?php } else { ?>
                ...
                <div class="col-sm-4">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <select class="offer_page_flow_navigation" name="offer_page_flows[<?php echo ($key + 1) ?>][destination_offer_page_id]" class="selectize">
                            <?php 
                                /* @var $page \Gun\OfferPage */
                                foreach ($offer_pages as $page) {
                            ?>
                                <option value="<?php echo $page->getId() ?>" <?php echo ($page->getId() == $offer_page_flow->getDestinationOfferPageId()) ? "SELECTED=\"selected\"" : "" ?> data-data="<?php echo htmlentities(json_encode(array('_id' => $page->getId(), 'name' => $page->getName(), 'description' => $page->getDescription(), 'page_name' => $page->getPageName(), 'preview_url' => $page->getPreviewUrl()))) ?>"><?php echo $page->getName() ?></option>
                            <?php } ?>
                            </select>
                            <p />
                            <a class="btn-sm btn-info text-center manage_destination_page" target="_blank" href="#">manage destination page &gt;&gt;</a>
    ...
    <div class="form-group">
        <div class="col-sm-offset-2 col-sm-10">
            <button type="button" class="btn btn-info" id="add_page_btn">Add Page</button>
