...
$(document).ready(function() {
	$('#server_search_form').on('submit', function(e) {
		$('#server_table').DataTable().clearPipeline().draw();
		e.preventDefault();
	});
	...
	$('#server_table').DataTable({
		...
		columns: [
			{ name: "hostname", data: "hostname", createdCell: function (td, cellData, rowData, row, col) {
				$(td).html('<a data-toggle="modal" data-target="#edit_server_modal" href="/admin/server-wizard?_id=' + rowData._id + '">' + cellData + '</a>');
			}},
