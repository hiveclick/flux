...
<ol class="breadcrumb small">
	...
	<li><a href="/report/revenue-report">Revenue Report</a></li>
</ol>
...
function loadRevenue() {
	$.rad.get('/api', {func: '/report/report-client', start_date: '<?php echo date('m/1/Y', $revenue_report->getReportDate()->sec) ?>', end_date: '<?php echo date('m/t/Y', $revenue_report->getReportDate()->sec) ?>', ignore_pagination: '1' }, function(data) {
		...
		if (data.entries) {
			$.each(data.entries, function(i, item) {
				if (item.revenue > 0) {
					...
					var div = $('<div class="text-right small" />').css('color', item.client.client_color).html('$' + $.number(item.revenue, 2));
					$daycell.after(div);
				}
		...
		$.each(revenue_total_array, function(i, item) {
			if (item.revenue > 0) {
				$( "#daytotal_" + item.day_of_year).html('$' + $.number(item.revenue, 2));
			} else {
				$( "#daytotal_" + item.day_of_year).css('color', '#C8C8C8').html('$' + $.number(item.revenue, 2));
			}
		});
		...
		$.each(revenue_array, function(i, item) {
			...
			$('<span class="badge" />').css('background-color', item.color).html('$' + $.number(item.revenue, 2)).prependTo(li);
			total_revenue += item.revenue;
		});
		$('#mtd_total_revenue,#mtd_total_revenue_top').html('$' + $.number(total_revenue, 2));

		// Draw the Network Pie Graph
		var rev_data_pie = [['Label', 'Revenue']];
		$.each(revenue_array, function(i, item) {
			rev_data_pie.push([item.name, {v:item.revenue, f:'$' + $.number(item.revenue, 2)}]);
		});
		...
		$.each(revenue_total_array, function(i, item) {
			daily_col_array.push([item.day_of_month, {v:item.revenue, f:'$' + $.number(item.revenue, 2)}]);
		...
		$('#highest_day,#highest_day_top').html('$' + $.number(high_day_item.revenue, 2));
		$('#lowest_day,#lowest_day_top').html('$' + $.number(low_day_item.revenue, 2));
		$('#average_day,#average_day_top').html('$' + $.number(avg_day_item.revenue, 2));
		$('#trending_rev,#trending_rev_top').html('$' + $.number(avg_day_item.revenue * <?php echo date("t", $revenue_report->getReportDate()->sec) ?>, 2));
	});
}
//-->
</script>