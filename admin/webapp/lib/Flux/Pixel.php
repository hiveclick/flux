...
class Pixel extends CommonForm {
	...
	function getOffer() {
		if (is_null($this->offer)) {
			...
			if (\MongoId::isValid($this->getLead()->getLeadId()) && \MongoId::isValid($this->getLead()->getLead()->getTracking()->getOffer()->getId())) {
				$this->offer->setOfferId($this->getLead()->getLead()->getTracking()->getOffer()->getId());
			...
			if (!$offer_found) {
				if ($this->getDomain() != '') {
					...
					if (count($offers) == 0) {
						...
						\Mojavi\Logging\LoggerManager::error(__METHOD__ . " :: " . "Found 1 offer (" . $this->offer->getName() . "), using it");
						...
						\Mojavi\Logging\LoggerManager::error(__METHOD__ . " :: " . "Found " . count($offers) . ", using the first one (" . $this->offer->getName() . ")");
						$offer_found = true;
					}
				} else {
