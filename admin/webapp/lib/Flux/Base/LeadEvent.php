...
class LeadEvent extends MojaviForm {
	 ...
	 */
	function getN() {
		return $this->getDataField()->getId();
	}
	...
	function setDataField($arg0) {
		if (is_array($arg0)) {
			...
			if ($data_field->getDataFieldKeyName() == '' && \MongoId::isValid($data_field->getId())) {
				$data_field->setDataFieldKeyName($data_field->getDataField()->getKeyName());
			}
			if ($data_field->getDataFieldKeyName() != '' && is_null($data_field->getId())) {
				$data_field->setDataFieldId($data_field->getDataField()->getId());
			}
			if ($data_field->getDataFieldKeyName() != '' && $data_field->getName() == "") {
