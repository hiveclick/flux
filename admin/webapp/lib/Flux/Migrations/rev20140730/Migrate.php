<?php
namespace Flux\Migrations\rev20140730;
...
class Migrate extends Migration {
	...
	function up() {
		...
		foreach ($data_fields as $data_field) {
			...
			if ($data_field->getKeyName() == \Flux\DataField::DATA_FIELD_REF_IP) {
			    $data_field->setKeyName('_ip');
			    $data_field->setRequestName(\Flux\DataField::DATA_FIELD_REF_IP);
			    $data_field->setStorageType(\Flux\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			    $data_field->update();
			} else if ($data_field->getKeyName() == \Flux\DataField::DATA_FIELD_REF_USER_AGENT_BROWSER) {
			    $data_field->setKeyName('_uab');
			    $data_field->setRequestName(\Flux\DataField::DATA_FIELD_REF_USER_AGENT_BROWSER);
			    $data_field->setStorageType(\Flux\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			    $data_field->update();
			} else if ($data_field->getKeyName() == \Flux\DataField::DATA_FIELD_REF_USER_AGENT_PLATFORM) {
			    $data_field->setKeyName('_uap');
			    $data_field->setRequestName(\Flux\DataField::DATA_FIELD_REF_USER_AGENT_PLATFORM);
			    $data_field->setStorageType(\Flux\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			    $data_field->update();
			} else if ($data_field->getKeyName() == \Flux\DataField::DATA_FIELD_REF_USER_AGENT_VERSION) {
			    $data_field->setKeyName('_uav');
			    $data_field->setRequestName(\Flux\DataField::DATA_FIELD_REF_USER_AGENT_VERSION);
			    $data_field->setStorageType(\Flux\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			    $data_field->update();
			    ...
			    $data_field->setStorageType(\Flux\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			    $data_field->update();
			} else if ($data_field->getKeyName() == \Flux\DataField::DATA_FIELD_REF_OFFER_ID) {
			    $data_field->setStorageType(\Flux\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			    $data_field->update();
			} else if ($data_field->getKeyName() == \Flux\DataField::DATA_FIELD_REF_CLIENT_ID) {
			    $data_field->setStorageType(\Flux\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			    $data_field->update();
