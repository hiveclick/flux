...
class SpyReport extends BaseReport {
	...
	public static function getColumns() {
		if(is_null(static::$columns)) {
			//@todo: create a new property for default column sort
			   ...
			   foreach(\Gun\DataField::retrieveActiveDataFields() AS $dataField) {
				$datafield_selector = '';
				if($dataField->retrieveValue('storage_type') === \Gun\DataField::DATA_FIELD_STORAGE_TYPE_MAIN) {
					$datafield_selector = $dataField->getKeyName();
				} else if($dataField->retrieveValue('storage_type') === \Gun\DataField::DATA_FIELD_STORAGE_TYPE_EVENT) {
					$datafield_selector = $dataField->getKeyName();
				} else {
					$datafield_selector = $dataField->getKeyName();
				}
	...
	public function runReport() {
		...
		$match = array();
		$match = array(
			\Gun\DataField::DATA_FIELD_EVENT_CONTAINER => array('$elemMatch' => array('t' => array(
		...
		if (count($this->getOfferId()) > 0) {
			$match[\Gun\DataField::DATA_FIELD_TRACKING_CONTAINER . '.' . \Gun\DataField::DATA_FIELD_REF_OFFER_ID . '._id'] = array('$in' => $this->getOfferId());
		}

		$lead = new \Gun\Lead();
		$lead->setSort(\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.t');
		$lead->setSord("DESC");
