<?php
namespace Gun\Migrations\rev20140505;
...
class Migrate extends Migration {
	...
	function up() {
		...
		$client = new \Gun\Client();
		$client = $client->query(array('status' => \Gun\Client::CLIENT_STATUS_ACTIVE), false);
		if (is_null($client)) {
			...
			$client->setStatus(\Gun\Client::CLIENT_STATUS_ACTIVE);
			$client->setClientType(\Gun\Client::CLIENT_TYPE_PRIMARY_ADMIN);
			$client->insert();
		}

		// Check if we have a first time user yet
		$user = new \Gun\User();
		$active_user = $user->query(array('status' => \Gun\User::USER_STATUS_ACTIVE), false);
		if (is_null($active_user)) {
			...
			$user->setUserType(\Gun\User::USER_TYPE_ADMIN);
			$user->setStatus(\Gun\User::USER_STATUS_ACTIVE);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_STRING);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_INTEGER);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
			$datafield->setRequestName('');
			$datafield->setTags('internal,tracking');
			$datafield->insert();
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			$datafield->setName('Client ID');
			$datafield->setKeyName(\Gun\DataField::DATA_FIELD_REF_CLIENT_ID);
			$datafield->setReportGroup(true);
			$datafield->setPixelAllowed(true);
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_ARRAY);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
			$datafield->setRequestName('');
			$datafield->setTags('internal,tracking');
			$datafield->insert();
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			$datafield->setName('Offer ID');
			$datafield->setKeyName(\Gun\DataField::DATA_FIELD_REF_OFFER_ID);
			$datafield->setReportGroup(true);
			$datafield->setPixelAllowed(true);
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_STRING);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_STATE);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_ZIP);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_COUNTRY);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_EMAIL);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_PHONE);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_GENDER);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_BIRTHDATE);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			...
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_URL);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
			$datafield->setTags('tracking');
			$datafield->insert();
		}
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			$datafield->setName('IP');
			$datafield->setKeyName(\Gun\DataField::DATA_FIELD_REF_IP);
			$datafield->setReportGroup(false);
			$datafield->setPixelAllowed(false);
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_IP);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			$datafield->setName('User Agent Browser');
			$datafield->setKeyName(\Gun\DataField::DATA_FIELD_REF_USER_AGENT_BROWSER);
			$datafield->setReportGroup(true);
			$datafield->setPixelAllowed(false);
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			$datafield->setName('User Agent Platform');
			$datafield->setKeyName(\Gun\DataField::DATA_FIELD_REF_USER_AGENT_PLATFORM);
			$datafield->setReportGroup(true);
			$datafield->setPixelAllowed(false);
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			$datafield->setName('User Agent Version');
			$datafield->setKeyName(\Gun\DataField::DATA_FIELD_REF_USER_AGENT_VERSION);
			$datafield->setReportGroup(true);
			$datafield->setPixelAllowed(false);
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			$datafield->setName('Created');
			$datafield->setKeyName(\Gun\DataField::DATA_FIELD_EVENT_CREATED_NAME);
			$datafield->setReportGroup(false);
			$datafield->setPixelAllowed(false);
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_EVENT);
			$datafield->setType(\Gun\DataField::DATA_FIELD_TYPE_DATETIME);
			$datafield->setAccessType(\Gun\DataField::DATA_FIELD_ACCESS_TYPE_PUBLIC);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			$datafield->setName('Updated');
			$datafield->setKeyName(\Gun\DataField::DATA_FIELD_EVENT_UPDATED_NAME);
			$datafield->setReportGroup(false);
			$datafield->setPixelAllowed(false);
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_EVENT);
		...
		if (is_null($datafield) || (!is_null($datafield) && $datafield->getId() == 0)) {
			$datafield->setName('Conversion');
			$datafield->setKeyName(\Gun\DataField::DATA_FIELD_EVENT_CONVERSION_NAME);
			$datafield->setReportGroup(false);
			$datafield->setPixelAllowed(false);
			$datafield->setStatus(\Gun\DataField::DATA_FIELD_STATUS_ACTIVE);
			$datafield->setStorageType(\Gun\DataField::DATA_FIELD_STORAGE_TYPE_EVENT);
		...
		$report_column = new \Gun\ReportColumn();
		...
		if (is_null($report_column) || (!is_null($report_column) && $report_column->getId() == 0)) {
			$report_column->setName('Impressions');
			$report_column->setStatus(\Gun\ReportColumn::REPORT_COLUMN_STATUS_ACTIVE);
			$report_column->setColumnType(\Gun\ReportColumn::REPORT_COLUMN_TYPE_GROUP_EVENT_SUM);
			$report_column->setSumType(\Gun\ReportColumn::COLUMN_SUM_VALUE);
			$report_column->setFormatType(\Gun\ReportColumn::COLUMN_FORMAT_DEFAULT);

			/* @var $created_datafield \Gun\DataField */
			$created_datafield = new \Gun\DataField();
		...
		if (is_null($report_column) || (!is_null($report_column) && $report_column->getId() == 0)) {
			...
			/* @var $partial_datafield \Gun\DataField */
			$partial_datafield = new \Gun\DataField();
		...
		if (is_null($report_column) || (!is_null($report_column) && $report_column->getId() == 0)) {
			...
			/* @var $conversion_datafield \Gun\DataField */
			$conversion_datafield = new \Gun\DataField();
		...
		if (is_null($report_column) || (!is_null($report_column) && $report_column->getId() == 0)) {
			...
			/* @var $upsell_view_datafield \Gun\DataField */
			$upsell_view_datafield = new \Gun\DataField();
		...
		if (is_null($report_column) || (!is_null($report_column) && $report_column->getId() == 0)) {
			...
			/* @var $upsell_partial_datafield \Gun\DataField */
			$upsell_partial_datafield = new \Gun\DataField();
		...
		if (is_null($report_column) || (!is_null($report_column) && $report_column->getId() == 0)) {
			...
			/* @var $upsell_conversion_datafield \Gun\DataField */
			$upsell_conversion_datafield = new \Gun\DataField();
		...
		$daemon = new \Gun\Daemon();
		$daemon->setType('Split');
		$daemon->query(array(), false);
		if (is_null($daemon) || (!is_null($daemon) && $daemon->getId() == 0)) {
			$daemon->setName('Split');
			$daemon->setStatus(\Gun\Daemon::DAEMON_STATUS_ACTIVE);
			$daemon->setRunStatus(\Gun\Daemon::DAEMON_RUN_STATUS_INACTIVE);
			$daemon->setThreads(2);
			$daemon->setClassName('Gun\Daemon\Split');
		...
		StringTools::consoleWrite('   - Building Gun Preferences Collection', 'Building', StringTools::CONSOLE_COLOR_YELLOW);

		\Gun\Preferences::ensureIndexes();

		StringTools::consoleWrite('   - Building Gun Preferences Collection', 'Done', StringTools::CONSOLE_COLOR_GREEN, true);
	}
