		...
		*/
		...
		foreach ($leads as $key => $lead) {
			...
			foreach ($this->getExport()->getClientExport()->getMapping() as $mapping) {
				$data_field = \Gun\DataField::retrieveDataFieldFromId($mapping['datafield_id']);
			...
			try {
				...
				$lead->setResponse('Sent');
				$lead->setIsError(false);
				$lead->update();
			} catch (\Exception $e) {
				$lead->setResponse($e->getMessage());
				$lead->setIsError(true);
				$lead->update();
			}
