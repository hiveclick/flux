...
class ClientExport extends MongoForm {
	...
	function getExportClass() {
		...
		if (trim($class_name) != '') {
			$class_name = '\\Gun\\Export\\' . $class_name;
			$ret_val = new $class_name();
			$ret_val->setClientExportId($this->getId());
		} else {
			$ret_val = new \Gun\Export\GenericFtp();
			$ret_val->setClientExportId($this->getId());
		}
		return $ret_val;
	}
	...
	function getTimezone() {
		if (is_null($this->timezone)) {
			$this->timezone = \Gun\Timezone::getDefaultTimezone();
		}
	...
	function sendLead($lead) {
		if ($this->getExportType() == self::CLIENT_EXPORT_TYPE_REALTIME) {
			return $this->sendLeadViaPostUrl($lead);
		} else {
			return $this->queueLeadViaFtp($lead);
		}
	}
	 ...
	 * @return boolean
	 */
	function sendLeadViaPostUrl($lead) {
		$post_url = $this->getPostUrl();
		$url = substr($post_url, 0, strpos($post_url, "?"));
		$qs = substr($post_url,strpos($post_url, "?") + 1);
		$params = array();
		parse_str($qs, $params);
		foreach ($params as $key => $param) {
			$data_field = DataField::retrieveDataFieldFromKeyName($key);
			if (!is_null($data_field)) {
				if ($data_field->getId() > 0) {
					if ($data_field->getStorageType() == \Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT) {
						if (isset($lead->getD()[$data_field->getKeyName()])) {
							$params[$key] = $lead->getD()[$data_field->getKeyName()];
						}
					} else if ($data_field->getStorageType() == \Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING) {
						if (isset($lead->getT()[$data_field->getKeyName()])) {
			 				$params[$key] = $lead->getT()[$data_field->getKeyName()];
			 			}
		...
		if (strpos($response->getBody(), $this->getSuccessMsg()) !== false) {
			return true;
		}
		return false;
	}
	 ...
	 * @return boolean
	 */
	private function queueLeadViaFtp($lead) {
