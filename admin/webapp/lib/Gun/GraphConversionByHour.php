...
class GraphConversionByHour extends GoogleChart {
	...
	function queryLeads() {
		...
		$ops[] = array(
			'$match' => array(
				\Gun\DataField::DATA_FIELD_EVENT_CONTAINER => array('$elemMatch' => 
					array('t' => 
						array(
		...
		$ops[] = array(
				'$match' => array(
						\Gun\DataField::DATA_FIELD_EVENT_CONTAINER => array('$elemMatch' =>
								array('n' => \Gun\DataField::DATA_FIELD_EVENT_CONVERSION_NAME)
						)
				)
		);
		...
		$ops[] = array(
				'$match' => array(
						\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.t' => array('$exists' => 1)
				)
		);
		...
		if (count($this->getOfferIdArray()) > 0) {
			$ops[] = array(
					'$match' => array(
							\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.o' => array('$in' => $this->getOfferIdArray())
					)
			);
		}
		
		$ops[] = array(
				'$unwind' => '$' . \Gun\DataField::DATA_FIELD_EVENT_CONTAINER 
		);
