...
class Lead extends MongoForm {
	...
	protected $_e; // array of \Gun\LeadEvent
	protected $_t; // \Gun\LeadTracking
	protected $notes;
	protected $_exports; // arra of \Gun\LeadExport
	...
	function populate($arg0, $modify_columns = true) {
		...
		if (is_array($arg0)) {
			foreach ($arg0 as $key => $value) {
			    ...
			    if (($key == \Gun\DataField::DATA_FIELD_DEFAULT_CONTAINER) && is_array($value)) {
                ...
                if (($key == \Gun\DataField::DATA_FIELD_TRACKING_CONTAINER) && is_array($value)) {
        			$this->setT($value);
        			if ($modify_columns) { $this->addModifiedColumn(\Gun\DataField::DATA_FIELD_TRACKING_CONTAINER); }
        			continue;
        		}
        		*/
			}
	...
	function setT($arg0) {
	    ...
	    } else if ($arg0 instanceof \Gun\LeadTracking) {
	        $this->_t = $arg0;
	    }
	...
	function getValue($data_name, $default_value = '', $assign_if_not_set = true) {
		$data_field = \Gun\DataField::retrieveDataFieldFromName($data_name);
	...
	function setValue($data_name, $data_value, $request_code = self::LEAD_REQUEST_CODE_UPDATE) {
		...
		if (!is_null($data_field)) {
			...
			if (in_array($request_code, array(self::LEAD_REQUEST_CODE_INC, self::LEAD_REQUEST_CODE_DEC))) {
				if (!in_array($data_field->getType(), \Gun\DataField::typesCanIncDec())) {
				...
				if (!in_array($data_field->getType(), \Gun\DataField::typesCanMaxMin())) {
			...
			if ($result === true) {
				if ($data_field->getStorageType() === \Gun\DataField::DATA_FIELD_STORAGE_TYPE_MAIN) {
					// right now we do nothing with these
				} else if($data_field->getStorageType() === \Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT) {
					// save the data
					$d = $this->getD();
	...
	protected function addEvent($event_key, $value = 1) {
	    LoggerManager::error(__METHOD__ . " :: " . "ADDING EVENT: " . VAR_EXPORT($event_key, true));
	...
	function queryAll(array $criteria = array(), $hydrate = true) {
		if (trim($this->getKeywords()) != '') {
			...
			$firstname_data_field = \Gun\DataField::retrieveDataFieldFromKeyName('fn');
			$lastname_data_field = \Gun\DataField::retrieveDataFieldFromKeyName('ln');
			if (strpos($this->getKeywords(), ' ') === false) {
				$search_params[] = array(\Gun\DataField::DATA_FIELD_DEFAULT_CONTAINER . '.' . $firstname_data_field->getKeyName() =>  new \MongoRegex("/" . $this->getKeywords() . "/i"));
				$search_params[] = array(\Gun\DataField::DATA_FIELD_DEFAULT_CONTAINER . '.' . $lastname_data_field->getKeyName() =>  new \MongoRegex("/" . $this->getKeywords() . "/i"));
			} else {
				...
				$search_params[] = array('$and' => array(
					array(\Gun\DataField::DATA_FIELD_DEFAULT_CONTAINER . '.' . $firstname_data_field->getKeyName() =>  new \MongoRegex("/" . $firstname . "/i")),
					array(\Gun\DataField::DATA_FIELD_DEFAULT_CONTAINER . '.' . $lastname_data_field->getKeyName() =>  new \MongoRegex("/" . $lastname . "/i"))
				));
			}
		...
		if (count($this->getRequiredFields()) > 0) {
			foreach ($this->getRequiredFields() as $required_field) {
				$data_field = \Gun\DataField::retrieveDataFieldFromKeyName($required_field);
				if ($data_field->getStorageType() == \Gun\DataField::DATA_FIELD_STORAGE_TYPE_DEFAULT) {
					$criteria[\Gun\DataField::DATA_FIELD_DEFAULT_CONTAINER . '.' . $data_field->getKeyName()] = array('$exists' => '1');
				} else if ($data_field->getStorageType() == \Gun\DataField::DATA_FIELD_STORAGE_TYPE_TRACKING) {
					$criteria[\Gun\DataField::DATA_FIELD_TRACKING_CONTAINER . '.' . $data_field->getKeyName()] = array('$exists' => '1');
				} else if ($data_field->getStorageType() == \Gun\DataField::DATA_FIELD_STORAGE_TYPE_EVENT) {
		...
		if (count($this->getOfferIdArray()) > 0) {
			$criteria[\Gun\DataField::DATA_FIELD_TRACKING_CONTAINER . '.' . \Gun\DataField::DATA_FIELD_REF_OFFER_ID . '._id'] = array('$in' => $this->getOfferIdArray());
		}
	...
	public static function ensureIndexes() {
		$lead = new \Gun\Lead();
		$lead->getCollection()->ensureIndex(array(\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.t' => -1, \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.o' => 1, \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.n' => 1), array('background' => true));
		$lead->getCollection()->ensureIndex(array(\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.c' => 1, \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.t' => -1, \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.o' => 1, \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.n' => 1), array('background' => true));
		$lead->getCollection()->ensureIndex(array(\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.o' => 1, \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.t' => -1, \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.n' => 1), array('background' => true));
		return true;
	}
}
