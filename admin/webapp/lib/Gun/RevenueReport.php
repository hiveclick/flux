...
class RevenueReport extends BaseReport {
	...
	public function getDateEventId() {
		if(is_null($this->date_event_id)) {
			$this->date_event_id = (string) \Gun\DataField::retrieveDataFieldFromName(\Gun\DataField::DATA_FIELD_EVENT_CREATED_NAME)->getId();
		}
	...
	public static function getColumns() {
		if (is_null(static::$columns)) {
			...
			$report_column_obj = new \Gun\ReportColumn();
	...
	public static function getGroups() {
		if(is_null(static::$groups)) {
			...
			static::$groups[] = array(
					'group_id' => 'geo',
					'group_selector' => \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.o',
					'text' => 'Event Offer',
					'type' => \Gun\DataField::DATA_FIELD_TYPE_OFFER_ID,
					'storage_type' => \Gun\DataField::DATA_FIELD_STORAGE_TYPE_EVENT
			);

			static::$groups[] = array(
					'group_id' => 'gec',
					'group_selector' => \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.c',
					'text' => 'Event Campaign',
					'type' => \Gun\DataField::DATA_FIELD_TYPE_CLIENT_ID,
					'storage_type' => \Gun\DataField::DATA_FIELD_STORAGE_TYPE_EVENT
			);

			foreach(\Gun\DataField::retrieveActiveGroupingDataFields() AS $dataField) {
				$group_selector = '';
				if ($dataField->getStorageType() === dataField::DATA_FIELD_STORAGE_TYPE_EVENT) {
					$group_selector = \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.' . $dataField->retrieveValue('_id');
				} else {
					//assume dataField::DATA_FIELD_DEFAULT_CONTAINER
					$group_selector = \Gun\DataField::DATA_FIELD_DEFAULT_CONTAINER . '.' . \Gun\Lead::LEAD_DATA_FIELD_PREFIX . $dataField->retrieveValue('_id');
				}
	...
	public function runReport() {
		...
		$match = array(
				\Gun\DataField::DATA_FIELD_EVENT_CONTAINER => array('$elemMatch' => array('t' => array('$gte' => new \MongoDate($this->retrieveValue('start_time_tz')->getTimestamp())))),
		);
		$explicit_match = array(
				\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.t' => array('$gte' => new \MongoDate($this->retrieveValue('start_time_tz')->getTimestamp()))
		);
		//if(isset($to)) {
		$match[\Gun\DataField::DATA_FIELD_EVENT_CONTAINER]['$elemMatch']['t']['$lt'] = new \MongoDate($this->retrieveValue('end_time_tz')->getTimestamp());
		$explicit_match[\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.t']['$lt'] = new \MongoDate($this->retrieveValue('end_time_tz')->getTimestamp());
		//}
		if(count($this->retrieveValue('offer_id')) > 0) {
			$match[\Gun\DataField::DATA_FIELD_EVENT_CONTAINER]['$elemMatch']['o'] = array('$in' => $this->retrieveValue('offer_id'));
			$explicit_match[\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.o'] = array('$in' => $this->retrieveValue('offer_id'));
		}
		if(count($this->retrieveValue('campaign_id')) > 0) {
			$match[\Gun\DataField::DATA_FIELD_EVENT_CONTAINER]['$elemMatch']['c'] = array('$in' => $this->retrieveValue('campaign_id'));
			$explicit_match[\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.c'] = array('$in' => $this->retrieveValue('campaign_id'));
		}
		if(count($this->retrieveValue('event_id')) > 0) {
			$match[\Gun\DataField::DATA_FIELD_EVENT_CONTAINER]['$elemMatch']['n'] = array('$in' => $this->retrieveValue('event_id'));
			if($this->retrieveValue('event_window') === self::EVENT_WINDOW_EXCLUSIVE_EXCPLICIT) {
				$explicit_match[\Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.n'] = array('$in' => $this->retrieveValue('event_id'));
			}
		}

		$query[] = array('$match' => $match);
		$query[] = array('$unwind' => '$' . \Gun\DataField::DATA_FIELD_EVENT_CONTAINER);
		...
		foreach($this->retrieveValue('column_id') AS $column_id) {
			...
			if (!is_null($report_column)) {
				...
				if($report_column->retrieveValue('type') === \Gun\ReportColumn::REPORT_COLUMN_TYPE_GROUP_DATAFIELD) {
					$event_id_array = $report_column->retrieveValue('parameter');
					if($report_column->retrieveValue('sum_type') === \Gun\ReportColumn::COLUMN_SUM_VALUE) {
						$event_array = array();
						foreach($event_id_array AS $event_id) {
							$event_array[] = array('$eq' => array('$' . \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.n', $event_id));
						}
						$group_initial_column_array[$column_name] = array('$max' => array('$cond' => array(array('$or' => $event_array), '$' . \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.v', 0)));
					} else if($report_column->retrieveValue('sum_type') === \Gun\ReportColumn::COLUMN_COUNT_TOTAL) {
						$event_array = array();
						foreach($event_id_array AS $event_id) {
							$event_array[] = array('$eq' => array('$' . \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.n', $event_id));
						}
						$group_initial_column_array[$column_name] = array('$max' => array('$cond' => array(array('$or' => $event_array), 1, 0)));
						//$group_array['$group'][$column_name] = array('$sum' => array('$cond' => array(array('$setEquals' => array(array('$' . \Gun\DataField::DATA_FIELD_EVENT_CONTAINER . '.n'), $event_id_array)), 1, 0)));
					}
				} else if($report_column->retrieveValue('type') === \Gun\ReportColumn::REPORT_COLUMN_TYPE_CALCULATION) {
					...
					$mongo_operator = \Gun\ReportColumn::convertOperatorTypeToMongo($report_column->retrieveValue('operator_type'));
