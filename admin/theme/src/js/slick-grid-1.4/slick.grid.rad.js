 ...
 * @param {Array}             footer      An array of column definitions to be used for a footer row|false.
...
(function($) {
    ...
    function SlickGrid($container,data,columns,options) {
        ...
        var footer_cols = (options.footer ? options.footer : false);
        var self = this;
        var $headerScroller;
        var $headers;
        var $footerScroller;
        ...
        var columnsById = {};
        var fColumnsById = {}; // footer columns by id
        ...
        function init() {
            ...
            /// </summary>
            gridData = data.rows || data;
            gridDataGetLength = data.getLength || defaultGetLength;
            gridDataGetItem = data.getItem || defaultGetItem;
            gridDataGetFooterItem = data.getFooterItem || defaultGetItem;
            ...
            if (footer_cols) {
	            $footerScroller = $("<div class='slick-footer panel-footer text-muted' style='overflow:hidden;position:relative;padding:0px;' />").appendTo($container);
	           	$footers = $("<div class='slick-footer-columns' style='width:100000px;' />").appendTo($footerScroller);
            }
            ...
            if (footer_cols) {
            	disableSelection($footers); // disable all text selection in header (including input and textarea)
            }
        ...
        function createColumnFooters() {
            var i;
            var footer_cols = getFooterCols();
            $footers.empty();
            fColumnsById = {};
                        
            for (i = 0; i < columns.length; i++) {
            	for (j = 0; j < footer_cols.length; j++) {
            		if (footer_cols[j].id == columns[i].id) {
            			// This is the matching footer, so use it
            			var m = footer_cols[j] = $.extend({},columnDefaults,footer_cols[j]);
    	                fColumnsById[m.id] = i;    	                
    	                var footer_html = $("<div class='" + m.cssClass + " panel-footing slick-footer-column' id='footer_" + uid + m.id + "' />")
        ...
        function getFooterColumnIndex(id) {
            return fColumnsById[id];
        }
        ...
        function styleColumnWidth(index,width,styleCells) {
            ...
            if (footer_cols) {
            	$footers.children().eq(index).css("width", width - headerColumnWidthDiff);
            }
            if (styleCells) {
        ...
        function setFooterCols(columnDefinitions) {
        	footer_cols = columnDefinitions;
        }
        ...
        function setData(newData,scrollToTop) {
            ...
            gridDataFooterItem = gridData.getFooterItem || [];
            if (scrollToTop)
                scrollTo(0);
        }
        ...
        function resizeCanvas() {
            ...
            var newViewportH = options.rowHeight * gridDataGetLength();
        ...
        function updateRowCount() {
            var newRowCount = gridDataGetLength();
            var oldH = h;
			
			// remove the rows that are now outside of the data range
			// this helps avoid redundant calls to .removeRow() when the size of the data decreased by thousands of rows
			var l = gridDataGetLength() - 1;
			for (var i in rowsCache) {
				if (i >= l) {
					removeRowFromCache(i);
				}
        ...
        function getRenderedRange(viewportTop) {
            ...
            range.bottom = Math.min(gridDataGetLength() - 1,range.bottom);
        ...
        function renderFooterRow() {
        	var d = gridDataGetFooterItem();
        	
        	if (d[0]) {
	        	for (var i=0; i<footer_cols.length; i++) {
	                var m = footer_cols[i];
	                // if there is a corresponding row (if not, this is the Add New row or this data hasn't been loaded yet)
                	formatter = m.formatter || defaultFormatter;
                	if (m.field.indexOf(".") == -1) {
                		val = formatter(1, i, d[0][m.field], m, d[0]);
                	} else {
                		var nested_value_function = function(obj, field) {
                			path=field.split('.');
                			var res=obj;
                		...
                		val = formatter(1, i, nested_value_function(d[0], m.field), m, d[0]);
                	}
                	$("#footer_" + uid + m.id).html("<span class='slick-column-name'>" + val + "</span>");
                }
            }        	
        }
        ...
        function render() {
            ...
            postProcessToRow = Math.min(gridDataGetLength(), visible.bottom);
            startPostProcessing();
